...
- **Regression tests**: formalize sanity checks already done ad hoc in
  earlier steps (`baseline/sanity_check.py`, `possession_value/fit_and_check.py`,
...
observed on real data in Step 2, is expected estimator behavior at this
sample size, not a red flag. The fixture uses 3x the data (1,140
...
caught in Step 3 that silently rounds r_hat to 2dp by default. Plus a
...
real-data monotonicity regression test from Step 4.
...
match throughout Steps 8-10; (2) a planted, real difference between two
