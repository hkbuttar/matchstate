# Step 11: Results & Honest Comparison

This is the synthesis of Steps 2-10: one place with the full comparison
table and direct answers to the four questions this project set out to
answer, stated plainly rather than spun toward a predetermined
conclusion. Every number here is a held-out or cross-validated result
with a citation back to the step that produced it -- nothing in this file
is a new claim, only an assembly of what was already found.
...
model's in its column -- consistent with Step 10's finding that 95
matches mostly can't distinguish these models from each other at the
per-class level. The one exception with real statistical teeth (Step 10,
combined 3-class): GBM is significantly worse than the Bayesian model
in-game, and the market is significantly better than GBM in-game.
...
(Step 6). Bootstrapped (Step 10): GBM is *significantly* worse than the
Bayesian model (diff +0.0267, CI [+0.005, +0.048]); not significantly
different from static.

But broken down by match phase (Step 6), GBM **wins clearly in the first
...
real information the Poisson-family models structurally can't see (Step
7 confirmed this concretely: GBM reacts to red cards, static/Bayesian
essentially don't, 0.157 vs. 0.005 mean swing at the red-card minute
across 16 real red cards). It loses ground steadily as the match
progresses, ending up worse in the final phase, traced to a specific,
demonstrated mechanism (Step 6): with only 285 training matches, GBM is
systematically underconfident on rare, extreme late-game states (a 2+
goal lead at minute 80 was a 100%-of-58-cases certain win in the test
set; GBM assigned it 79-95%, static Dixon-Coles correctly assigned
97-99.9%).
...
data-volume-limited outcome Step 1's disclosed 380-match StatsBomb
ceiling predicted was possible.
...
(Step 9): Brier 0.585 vs. our best (static) at 0.593, a small (~1.4%
...
(Step 9's phase breakdown), but when bootstrapped (Step 10) it is **not
...
explicitly.** By raw Brier score and calibration error (Step 8), draw is
...
seasons (Step 3), the dynamic model has lower Brier score than static
...
the single-season in-game test set (Step 10): the gap (Bayesian - static
...
different statistical power. 33 seasons of directional evidence (Step 3)
is suggestive; 95 matches of rigorous bootstrap evidence (Step 10) is not
enough to confirm it at 95% confidence. Where this project *does* have
strong, tightly-bounded evidence on a closely related question (Step
10's cross-season walk-forward, 8,808 matches): using **last season's**
strength to predict **this season** is significantly worse than fitting
on the season itself (+0.0336 Brier, CI [+0.029, +0.038], unambiguous).
That's not quite the same claim as "within-season updating helps," but
it's a rigorous confirmation of the underlying premise Step 3 was built
on -- team strength genuinely does shift in ways a single fixed estimate
misses, at least across season boundaries. The within-season version of
that claim is directionally supported but not proven at the confidence
this project's single-season event dataset can deliver.
...
dismissed, but it doesn't meet the higher bar Step 10 rightly holds it to
on the smaller, event-level-matched sample.
...
disclosed as far back as Step 1: **StatsBomb's free open data covers
